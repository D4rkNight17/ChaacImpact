...
const LANG_TEXTS = {
  es: {
    ...
    searchError: "⚠️ Error al conectar con el servidor.",
  ...
  en: {
    ...
    searchError: "⚠️ Error connecting to server.",
...
async function buscarAsteroides(nombre) {
  ...
  try {
    // 👇 Conexión con backend local Laravel
    const url = `http://127.0.0.1:8000/api/asteroids/${encodeURIComponent(nombre)}`;
    ...
    const result = {
      name: a.name,
      estimated_diameter: { meters: { estimated_diameter_max: a.diameter } },
      close_approach_data: [
        { relative_velocity: { kilometers_per_second: a.velocity } },
      ],
    };

    cacheAsteroides[key] = result;
...
function animarImpacto(coords, D_f_m, E_Mt, R_e_m) {
  ...
  const lonOffset0 = 6.0;
  const latOffset0 = 3.0;
